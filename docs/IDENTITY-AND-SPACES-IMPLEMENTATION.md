...
**Current status, 21 September 2026:** Restricted designated-account pilot is live. Real temporary/trusted sign-in, password recovery/revocation, logout, owner claim, My space with a 1 GiB total budget, library switching and explicit publication are verified. Independent backup/restore and isolated hosted publication race checks passed. General onboarding remains closed while operational and deletion/retention release gates are completed. The earlier dated entries below are historical implementation records; the [roadmap](DEVELOPMENT-ROADMAP.md) and [release gates](PHASE-2-RELEASE-GATES.md) track current completion.
...
### Pilot activation and provider compatibility result

The pilot is now live at Worker `0a65245e-5af3-4295-aa37-136c68d493be` (100%). The public file is enabled/pilot, with zero personal allocation; private Worker bindings hold the client secret, recovery secret and designated subject list. Migrations through 0018 are remote and all legacy rows were compared unchanged after migration. A fresh independent backup restored all 20 originals before mutation; [deployment evidence](../deploy/STATUS.md) records the recovery point and versions.

Auth0's actual Node 22 hosted sandbox rejected `await import('node:crypto')` with `ERR_VM_DYNAMIC_IMPORT_CALLBACK_MISSING`. The Action now uses `require('node:crypto')`, consistent with its CommonJS environment. Local tests execute the source in a VM without a dynamic-import callback so this incompatibility cannot be hidden by the local native module loader. The ESLint exception is limited to Auth0 CommonJS Action sources. The corrected hosted source exactly matches the repository (LF SHA-256 `39a8bee9a64c700b3f8073df53dae0583527c51adb509b94d997267e3fe7a4e1`).

The corrected hosted test completed with no error in 316 ms against the live receiver, proving the saved Action and Worker secrets agree. Independent signed/unsigned synthetic probes returned 204/403. Only a synthetic recovery watermark was recorded; no user password changed or account access granted. This is provider-to-receiver verification, not the real account-recovery acceptance test. The notification version is deployed; user trigger binding and real sign-in remain pending. Hosted test errors are visible and redacted; continuous production failure alerting/reconciliation is still an open gate.

## Free-plan readback - 21 September 2026

Auth0 team subscription settings show current plan Free / $0. Its Branding comparison explicitly marks Custom Email Provider as Included, while Email Workflow/template customisation is marked Trial. This distinction resolves the ambiguous public pricing label: the native Resend provider itself is included. Relay must not make successful recovery depend on paid template customisation. Database authentication and Actions Library are included. Two deployed Actions fit the published Free allowance of five; the temporary displayed Actions + Forms limit of 30 is a trial entitlement and is not assumed after expiry.

Initial sign-in remains verified email/password only. Google is disabled for Relay Web because its connection uses development keys; passkeys/social expansion requires its own configuration and verification. No plan or billing setting was changed. Actual post-trial behaviour remains a dated operational verification item; the dashboard still shows 20 trial days. [Published comparison](https://auth0.com/pricing) was checked alongside the signed-in team settings.

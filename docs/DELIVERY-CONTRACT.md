...
### Browser file-writer and compatibility acceptance

- [x] Verify real disk-backed Chromium file writers with generated 80 MiB and full 2 GiB ZIPs. The save picker is substituted with a disposable origin-private file handle; actual FileSystemWritableFileStream writes/close/abort are used. Cancelling replacement preserves the previously committed generated archive.
- [x] Preserve the application's CSP in the fixture: a local proxy serves generated bytes on the same test origin. A separate temporary persistent test profile avoids incognito quota limits; no normal user profile or existing user file is opened.
- [x] Verify small-package fallback downloads, manifest contents, stale-access denial and focus return in Chromium, Firefox and WebKit. Correct WebKit focus restoration and attach/remove the fallback anchor for consistent browser behavior.
- [x] Additional coverage passes both jobs in hosted run `35754168633` at `a0e6306`.
- [ ] Live provider download/expiry/retry and production activation remain separate gates.

These results supersede the earlier outstanding full-size local browser-writer check. They do not claim testing on every mobile device or a real system picker interaction.

...
- [ ] Pass hosted CI for the migration/configuration increment.
- [ ] Apply only migration 0019 after a fresh recovery point; export again and independently confirm every pre-existing row remains unchanged.
- [ ] Install a separate random coordinator secret into the existing Worker and protected `relay-backup-copy` environment; retain encrypted local custody. Do not reuse the monitoring secret or disclose credentials.

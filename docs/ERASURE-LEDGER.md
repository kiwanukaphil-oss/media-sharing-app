...
## Independent journal reconstruction

`node --disable-warning=ExperimentalWarning scripts/restore-erasure-journal.mjs` uses the repository-pinned public root and existing read-only B2 role to recover every authenticated revision into a new private SQLite database. It never opens the operational journal or application database and does not access signing custody. Existing destinations, changed destinations, incomplete history and competing signed revisions fail closed. Audit finishes before importing history; inserts/head advancement commit together.

Tests preserve pending and withdrawn decisions after the original database is lost, including expired manifests and replayed uploads. The real cloud rehearsal on 22 September restored the initial empty revision into a unique ignored review directory. This proves archive reconstruction, not current live-intent completeness: the output is `currentnessVerified: false`, `writerActivated: false`, `cutoverAllowed: false`. No recovered journal is automatically promoted.

## Recorded request and withdrawal producer

`record-erasure-intents.mjs` can append only pending, review-required and withdrawn observations. The complete live projection retains every request and binds it to its exact issuer/subject digest. Missing earlier rows, changed identity/timestamps, overlapping requests, rollback and fulfilled input stop for review. A request withdrawn before the first observation yields its retained pending and withdrawal timestamps as separate signed history entries. Per-revision atomic appends retain a valid prefix after interruption; replay adds only the missing transitions. A freshness renewal preserves record times.

`prepare-live-erasure-intents.mjs` reuses the existing read-only D1 credential, reads the fixed bounded projection twice and saves an unsigned private plan only when observations agree within 30 seconds. The real 22 September run found zero requests. Matching observations do not freeze writes or prevent a later withdrawal. The signer is now connected to the single-operator synchronization command described below; application/backup write fencing remains separate. The code has no fulfilment or deletion path.

## Single-operator synchronization

`node --disable-warning=ExperimentalWarning scripts/sync-erasure-intents.mjs` holds the same exclusive local writer lock as bootstrap, verifies owner custody against the repository-pinned root, reads complete live intent and records only pending/review/withdrawn transitions. It audits existing remote versions against local signed history, uploads missing immutable revisions, independently reads the current head and repeats the source observation before reporting success. A partial failure leaves its authenticated prefix and private evidence for retry; remote divergence stops for review.

This is an operator command, not a newly installed background service. The existing owner PC is the sole operational signer. A lock left by a crash must be investigated; another host must not be activated while it may still be signing. This local lock is not distributed consensus or a backup-writer freeze. No private key was sent to Cloudflare, GitHub or the application.

The real 22 September run found zero requests, renewed the expired empty manifest to revision 2, and independently verified both retained revisions. A separate read-only CLI confirmed the same head. The one-hour manifest expiry remains enforced; callers must obtain fresh head evidence when needed. No fulfilled decision, account deletion or restore clearance is produced.

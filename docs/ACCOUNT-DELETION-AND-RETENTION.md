...
## Transfer commit authority

Transfer reservations, upload completion and thumbnail commits now recheck live authority in the same D1 mutation. An account request resolved before revocation cannot commit afterwards using its cached principal: current session expiry/revocation, recovery watermark, account disablement, membership and personal ownership are required. Legacy transfers require current device access and cannot target personal spaces. A rejected upload reservation aborts its new multipart allocation; a rejected thumbnail commit removes its generated preview and releases the reservation. An already completed R2 original whose database publication is rejected remains an unfinished transfer for explicit reconciliation, rather than being silently exposed or destroying shared content.

This prerequisite is deployed in Worker `59b996f4-aea4-4ece-a808-3798a2d783b3`, with real-D1 tests, hosted CI and isolated hosted multipart verification, but is **not a complete write freeze**. Already-issued direct upload URLs, in-flight R2 requests, publication jobs, other metadata routes and independent backup writers still require coordinated fencing and reconciliation before physical erasure. Deployment evidence is recorded separately in the roadmap.

The [closure execution contract](ACCOUNT-CLOSURE-EXECUTION.md) specifies the user-facing cancellation boundary, durable writer fencing, independent backup coordination and interruption/retry acceptance. A one-hour URL expiry or matching repeated listings must never be presented as proof that every writer has stopped.

Account deletion preview/request/withdrawal now also check the current credential-change watermark in their database authority predicates, preventing cached pre-recovery sessions from changing intent. Withdrawal timestamps advance monotonically. These changes are deployed in Worker `2241047a-0bd9-441f-b0fd-bf301c4004f5`; actual-D1 tests cover the cached-session race and preserve the pending request on denial. This is an intent-authority check, not an erasure executor.

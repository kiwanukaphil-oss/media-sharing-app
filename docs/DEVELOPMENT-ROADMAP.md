...
**Next action:** User signs in in the prepared Relay Web tab and connects the tested recovery notification in Post Change Password. Then verify real callback, session revocation after password recovery, and provider logout. One recovery email is authorised but has not been sent. Recovery notification passed its hosted synthetic test after correcting an Auth0 CommonJS compatibility issue. Automated failure monitoring, deletion operations, personal budget, live membership/publication checks and general release remain outstanding. See [Phase 2 release gates](PHASE-2-RELEASE-GATES.md). The original verification message went to Spam; fresh email authentication and inbox placement still require the recovery test. B02 remains resolved.
...
- [ ] User completes real pilot sign-in and applies the prepared password-change trigger. General activation and personal allocation remain closed pending release gates.

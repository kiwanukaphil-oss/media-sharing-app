...
- [ ] Observe the next scheduled monitor on Paid; deployed version `679cc9bd` has independently verified 1,000 ms CPU limit and retains the 40-identity pilot guard. No broad capacity claim.
...
- [ ] User-dependent live check: sign in in the existing Relay Web tab; no password should be sent in chat. Current production health/access checks pass, but fresh signed-in interactive verification remains incomplete.

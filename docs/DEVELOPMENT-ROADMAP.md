...
- [x] Existing live browser regression, deployment readback, signed-in album/personal/shared navigation and both-origin health pass. Worker `5919b3c0-0e51-4ec8-a467-5669171b31d1` serves runtime `3ff3193` at 100%; schema stays 0020. Existing phase counts remain unchanged.
...
- [x] Integrate the approved direction into the authenticated app and complete live UI release acceptance; see the release evidence above. Schema and backend activation remain unchanged.
...
- [x] Adapt the reviewed direction to the authenticated app, preserve access/deep links/transfers/sections/clients, and verify responsive and browser/API regressions; released as documented above.
...
- [x] Implement reviewed owner-only snapshots, named-recipient invitation acceptance, sender/recipient web workflows, bounded original downloads and sticky revocation.
- [x] Verify current source authority, no source membership/disclosure, captured public labels, actual original bytes and mobile/desktop workflows.
- [x] Prepare migration 0026, exact-person minimisation, source-removal metadata cleanup and permanent restored-link quarantine; private-export rehearsal preserves all 25 existing tables.
- [x] Hosted source `c0a5151` passes both jobs in [35749765799](https://github.com/kiwanukaphil-oss/media-sharing-app/actions/runs/35749765799); merged into main with delivery activation disabled.
- [x] Implement bounded original ZIP packages and manifests on `work/snapshot-deliveries` at `15bbe8e`; local independent extraction, bounded streaming, cancellation/corruption/access-loss and library Viewer/recipient browser checks pass.
...
- [x] Main intake merge `f7f175d` passed both jobs in run `35746222158`. Earlier hosted migration/review/lifecycle source `269c1ef` passed both jobs in [35745353865](https://github.com/kiwanukaphil-oss/media-sharing-app/actions/runs/35745353865). Subsequent refresh/clock correction `1000fd7` passes local lint, TypeScript and browser checks.
...
**At a glance:** 2 phases complete; 22/47 parent work items complete. Phase 2 remains a restricted designated-account pilot with one 100 GiB allowance combined across the existing personal/shared libraries; general onboarding and account closure are not enabled. Live Worker `5919b3c0-0e51-4ec8-a467-5669171b31d1` is at 100%, sourced from the schema-compatible album-library branch at `3ff3193`. Production remains on migration 0020; unreleased migrations 0021-0027 and their dependent backend changes remain preserved on `main` but are not deployed. Global backup coordination is enabled and verified: run `35703394048` independently restored all 25 originals (321,680,743 bytes), and separate D1/B2 readers verified its archived completion receipt against the live settled run. Both-origin health/access checks and a fresh signed-in library reload pass; the existing three active shared files, one Trash item and album remain available. No user original was removed.

**Next action:** Finish remaining local acceptance and release-readiness review, then release only after the independent backup/provider gates clear. Duplicate assistance passed hosted run `35752042399` and is merged into main. Restricted audiences, intake and delivery workflows are locally implemented and hosted-verified; live activation remains gated. Independent backup downloads resumed: full run `35814505652` and incremental run `35821232068` passed on 23 September. Favourites/activity may proceed through migration and live acceptance; provider disposition remains a separate gate. Keep account closure/general release gated on actual lifecycle evidence; backup coordination and current-pilot monitoring are already verified. The Better Stack identity/recovery test email is confirmed in Gmail Inbox (21 September, 11:57 a.m. EAT; inspected 22 September). Independent scheduled checks passed at 15:19, 15:49 and 16:19 UTC; optimised runs used 8 ms and 5 ms CPU on Workers Free; the 22 September 08:19 run reached 10 ms. Workers Paid is now explicitly approved and active, with the bounded monitor deployed; the 08:49 UTC Paid cron and independent report delivery passed at 8 ms CPU against its 1,000 ms limit. Live personal upload, deliberate publication and independent backup verification passed. Isolated cloud interruption/retry, revocation and cancellation checks also passed; the rehearsal endpoint is disabled. The designated-account pilot remains restricted while lifecycle and general-release gates remain open. See [Phase 2 release gates](PHASE-2-RELEASE-GATES.md).
...
| --- | --- | --- | --- | --- | --- |
| B06 | Additive release / combined capacity | Historical B2 cap failures on 22 September; new payment/caps now operational. | Codex: use verified recovery evidence for staged release | 2026-09-23 | Resolved: full `35814505652` and incremental `35821232068` passed. Combined pool deployed and browser-verified. Older failed snapshots remain unverified. |
...
- [x] Hosted admission-pause source `9e8fb16` passes both jobs in run `35752698764`; merged into main.
- [ ] Independent backup/provider gates and Phase 6 demand validation remain outstanding.
...
- [x] Full backup `35814505652` independently reread 323,643,069 original bytes; daily incremental acceptance `35821232068` validated exact versions against authenticated evidence without redownloading unchanged originals. B06 resolved.
- [x] Deploy schema-0020 compatible source `cc8649d`, private two-library configuration, Worker `37d3cc5f-9811-4ddc-94f0-fa00a82e9767` at 100%, 05:11 UTC. Both-origin health/security/anonymous-denial probes pass.
- [x] Signed-in Personal and Shared storage dialogs both show 132.7 MB used of one combined 100.0 GB allowance; library contents and Trash details remain separate.
...
- [x] Isolate the Cobalt presentation and functional copy onto schema-0020-compatible source `25c2096` after the first deployment exposed unreleased migration dependencies. Deploy Worker `fd0a29f5-4aaa-4e5e-ba49-ef8d890bae35` at 100%; both origins, the real signed-in library, the live stylesheet and the complete local browser matrix pass. No schema, data or binding change occurred.

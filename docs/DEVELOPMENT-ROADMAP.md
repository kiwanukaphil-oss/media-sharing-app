...
**At a glance:** 2 phases complete; 22/47 parent work items complete. Phase 2 remains a restricted designated-account pilot with one 100 GiB allowance combined across the existing personal/shared libraries; general onboarding and account closure are not enabled. Live Worker `27898165-96ee-4e28-a1ef-a428a9cd73f9` is at 100%, including the Cobalt UI, its mobile heading-wrap correction, reliable Chromium folder reselection and delayed recovery identity protection. Migration 0019 preserved every existing row across all 21 prior application tables. Global backup coordination is enabled and verified: run `35703394048` independently restored all 25 originals (321,680,743 bytes), and separate D1/B2 readers verified its archived completion receipt against the live settled run. Both-origin health/access checks pass. Fresh read-only storage reconciliation recorded 33 objects, zero unfinished uploads and zero anomalies. Hosted Cobalt run `35952943796` passes verification and the complete browser matrix. Fresh sign-in, personal preview, recovered-publication history and shared-library switching are verified on 22 September; the database still contains 25 ready files and one publication. No user original was removed.
...
- [x] Merge `ui/cobalt-refresh` to `main` at `f300894`, apply the mobile heading-wrap correction at `9269935`, fix repeated Chromium folder selection at `37620f4` and deploy Worker `27898165-96ee-4e28-a1ef-a428a9cd73f9` at 100%. Hosted run `35952943796`, both origins and the live Cobalt stylesheet pass; schema and runtime settings remain unchanged.

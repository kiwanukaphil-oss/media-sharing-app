...
**22 September:** `work/restricted-scopes` contains locally tested partial runtime integration and additive migration 0023. It is isolated from the main release candidate and is not approved for deployment. File/album/section reads and mutations, favourites, exports, retained activity and scoped upload retries now have local D1 coverage. Scope creation remains unavailable through routes. Recovery and minimisation rehearsals passed; management/navigation UI, cross-scope copy and full browser/release checks remain outstanding. Main remains the separate release candidate held for independent backup download capacity. [Checked implementation milestones](RESTRICTED-ALBUMS-IMPLEMENTATION.md#isolated-runtime-integration--22-september-2026). Parent counts remain unchanged.

...
async function verifyCallbackOrchestration(database) {
  ...
  assert.equal(wrongBrowser.headers.get('Location'), settings.appOrigin + '/login?signin=failed');
  ...
  assert.equal(duplicated.headers.get('Location'), settings.appOrigin + '/login?signin=failed');
  ...
  assert.equal(results.filter(result => result.headers.get('Location') === settings.appOrigin + '/workspaces').length, 1);
  assert.equal(exchanges, 1);
  const success = results.find(result => result.headers.get('Location') === settings.appOrigin + '/workspaces');
  assert.equal(success.headers.get('Location'), settings.appOrigin + '/workspaces');

...
try {
  ...
  assert.match(await form.text(),/type="password"/);
  ...
  for(const body of [valid+'&password=duplicate',valid.replace('saved=yes','saved=no'),valid.replace('confirmation='+password,'confirmation=wrong'),'x'.repeat(4097)]) {

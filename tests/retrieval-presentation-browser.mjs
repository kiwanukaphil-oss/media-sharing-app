...
try {
  ...
  await page.getByLabel('Save the current view').fill('My other files');
  ...
  await page.getByRole('button',{name:'Saved views',exact:true}).click();
  await page.getByRole('button',{name:'My other files',exact:true}).click();
  await expect(page.getByRole('combobox',{name:'Added by',exact:true})).toHaveAttribute('data-value','me');
  await page.getByRole('button',{name:'Clear all filters',exact:true}).click();
  await page.getByRole('button',{name:'← Back to albums',exact:true}).click();
  await page.getByRole('button',{name:'Open album Recent album fixture'}).click();
  await expect(page.getByRole('heading',{name:'Recent album fixture'})).toBeVisible();

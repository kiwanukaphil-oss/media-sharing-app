...
import { ArrowDown, ArrowDownToLine, ArrowLeftRight, ArrowUpRight, Check, CheckCheck, ChevronRight, CircleHelp, Clapperboard, Copy, FileImage, FileVideo, FolderInput, Folder, FolderDown, Grid2X2, ImagePlus, Laptop, Link2, List, LoaderCircle, Menu, MonitorSmartphone, MoreHorizontal, Pause, Pencil, Play, Plus, Radio, RefreshCw, Search, Send, ShieldCheck, Smartphone, Star, Trash2, Undo2, Upload, Users, X } from "lucide-react";
...
function RelayWorkspace() {
  ...
  const refreshAlbums = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      if (!signal?.aborted && scope === audienceRef.current) setAlbumFailure(failure instanceof Error ? failure.message : "Please try again.");
      throw failure;
    }
  }, [requestJson]);
  ...
  useEffect(() => {
    ...
    const restoreLibraryLocation = () => {
      ...
      const explicitFileRoute = params.get("view") === "files" || Boolean(restored.album || restored.from || restored.to || restored.batch || restored.type || restored.uploader || restored.favorites || params.get("q") || category !== "all");
  ...
  async function openCreatedAlbum(id: string, message: string) {
    ...
    changeLibraryQuery({ ...emptyLibraryQuery, scope: libraryQuery.scope, album: id });
  ...
  return <div className={`app-shell album-first ${librarySurface === "albums" ? "albums-arrival" : "album-files"}${currentAlbum ? " in-album" : ""} ${transfers.length || download ? "has-transfers" : ""}`} onDragEnter={event => { if (canUpload && librarySurface === "files" && event.dataTransfer.types.includes("Files")) { event.preventDefault(); dragDepth.current++; setDragging(true); } }} onDragOver={event => event.preventDefault()} onDragLeave={event => { event.preventDefault(); dragDepth.current--; if (dragDepth.current <= 0) setDragging(false); }} onDrop={event => { event.preventDefault(); dragDepth.current = 0; setDragging(false); if (canUpload) void selectOriginals(event.dataTransfer.files); }}>
    ...
    <div className="workspace" inert={navigationOpen}>
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        {librarySurface === "files" || !session ? <div className="page-heading"><div className="album-title-group">{currentAlbum && <AlbumCover album={currentAlbum} colour={appearance[currentAlbum.id]?.colour} compact />}<h1>{pageTitle}<span className="title-dot">.</span></h1></div>{filter !== "trash" && canUpload && <div className="page-upload-actions">{isOrganiser && !currentAlbum && <FolderImport accessScopeId={libraryQuery.scope || null} audienceName={libraryQuery.scope ? "Restricted audience" : "General library"} disabled={!session || session.transport === "unconfigured"} spaceName={session?.space.name || "This library"} queue={queueFolderImport} />}<button className="button primary" disabled={!session || session.transport === "unconfigured" || Boolean(currentAlbum?.archivedAt)} onClick={() => fileInput.current?.click()}><Plus size={18} />Add files</button></div>}{filter === "trash" && <button className="button secondary" onClick={() => setFilter("all")}>Back to files</button>}</div> : null}
        ...
        {loading ? <div className="loading-panel"><LoaderCircle className="spin" size={25} /><p>Opening your space…</p></div> : sessionFailure && !session ? <section className="loading-panel"><p>Couldn&apos;t open your space.</p>{accountSpaceId !== undefined && <a className="button secondary" href="/account">Sign in or choose a library</a>}<button className="button secondary" onClick={() => void loadSession()}>Try again</button></section> : invitationRequired ?
          ...
          <LibraryTools managementTools={isOrganiser && currentAlbum ? <FolderImport accessScopeId={libraryQuery.scope || null} audienceName={libraryQuery.scope ? "Restricted audience" : "General library"} disabled={!session || session.transport === "unconfigured"} spaceName={session?.space.name || "This library"} queue={queueFolderImport} /> : undefined} onMoveSection={setSectionMoveItems} onCreateDelivery={session.deliveries && isOwner && filter !== "trash" ? () => setModal("delivery-create") : undefined} feedback={{ message: feedbackMessage, setMessage: setFeedbackMessage, undo: undoLibraryAction, setUndo: setUndoLibraryAction }} albums={albums} sections={sections} query={libraryQuery} onQuery={query => { if (query.album && query.section !== libraryQuery.section) setFilter("all"); setSectionMoveItems([]); changeLibraryQuery(query); }} canOrganise={isOrganiser && libraryQuery.scope !== "accessible"} canSelectFiles={canSelectFiles} canEditFiles={canEditFiles} canFavorite={session.authentication === "account"} selected={visibleItems.filter(item => selectedIds.has(item.id))} loadedCount={visibleItems.length} onSelectLoaded={() => setSelectedIds(new Set(visibleItems.slice(0, 100).map(item => item.id)))} onClearSelection={() => setSelectedIds(new Set())} refresh={refreshLibrary} renameItems={renameItems} onRename={setRenameItems} dateItem={dateItem} onDate={setDateItem} viewControls={<><SavedLibraryViews key={session.deviceId} storageKey={savedLibraryViewsKey(session.space.id, session.deviceId, session.authentication || "legacy")} current={{ category: filter, search, query: libraryQuery }} apply={view => { setFilter(view.category); setSearch(view.search); changeLibraryQuery(view.query); }} />{!currentAlbum && fileViewControls}</>}>
    ...
    {dragging && librarySurface === "files" && canUpload && filter !== "trash" && <div className="drop-overlay"><Upload size={52} /><h2>Drop files to add</h2></div>}

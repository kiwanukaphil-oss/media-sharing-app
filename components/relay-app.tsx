...
import { ArrowDown, ArrowDownToLine, ArrowLeftRight, ArrowUpRight, Check, CheckCheck, ChevronRight, CircleHelp, Clapperboard, Copy, FileImage, FileVideo, Folder, FolderDown, Grid2X2, ImagePlus, Laptop, Link2, List, LoaderCircle, Menu, MonitorSmartphone, MoreHorizontal, Pause, Pencil, Play, Plus, Radio, RefreshCw, Search, Send, ShieldCheck, Smartphone, Trash2, Undo2, Upload, Users, X } from "lucide-react";
...
function RelayWorkspace() {
  ...
  }, [accountSpaceId, filter, search, libraryQuery, librarySurface, session, refreshFeed]);
  useEffect(() => {
  ...
  return <div className={`app-shell album-first ${librarySurface === "albums" ? "albums-arrival" : "album-files"} ${transfers.length || download ? "has-transfers" : ""}`} onDragEnter={event => { if (canUpload && librarySurface === "files" && event.dataTransfer.types.includes("Files")) { event.preventDefault(); dragDepth.current++; setDragging(true); } }} onDragOver={event => event.preventDefault()} onDragLeave={event => { event.preventDefault(); dragDepth.current--; if (dragDepth.current <= 0) setDragging(false); }} onDrop={event => { event.preventDefault(); dragDepth.current = 0; setDragging(false); if (canUpload) void selectOriginals(event.dataTransfer.files); }}>
    ...
    <aside ref={sidebarRef} className={`sidebar${navigationOpen ? " is-open" : ""}`} role={navigationOpen ? "dialog" : undefined} aria-modal={navigationOpen || undefined} aria-label="Workspace navigation" onClick={event => { if ((event.target as HTMLElement).closest("button") && !(event.target as HTMLElement).closest(".library-switcher")) setNavigationOpen(false); }}>
      ...
      {session && <><p className="nav-label albums-label">ALBUMS</p><nav className="album-navigation" aria-label="Albums">
        {albums.filter(album => !album.archivedAt).map(album => <button className={`nav-item${libraryQuery.album === album.id ? " active" : ""}`} aria-current={libraryQuery.album === album.id ? "page" : undefined} key={album.id} onClick={() => { setFilter("all"); setSearch(""); changeLibraryQuery({ ...emptyLibraryQuery, album: album.id }); }}><Folder size={18} /><span className="album-name">{album.name}</span><span>{album.count}</span></button>)}
    ...
    <div className="workspace" inert={navigationOpen}>
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        {loading ? <div className="loading-panel"><LoaderCircle className="spin" size={25} /><p>Opening your space…</p></div> : sessionFailure && !session ? <section className="loading-panel"><p>Couldn&apos;t open your space.</p>{accountSpaceId !== undefined && <a className="button secondary" href="/account">Sign in or choose a library</a>}<button className="button secondary" onClick={() => void loadSession()}>Try again</button></section> : invitationRequired ?
          ...
          {currentAlbum && <div className="album-destination" role="status"><Folder size={16} /><span>{session.space.name} / {currentAlbum.name}{libraryQuery.section ? ` / ${sections.find(section => section.id === libraryQuery.section)?.name || "Unsectioned"}` : ""}</span>{currentAlbum.archivedAt ? <strong>Archived · unarchive to add files</strong> : <strong>Upload destination</strong>}</div>}

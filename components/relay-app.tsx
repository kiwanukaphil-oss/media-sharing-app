...
/* eslint-disable @next/next/no-location-assign-relative-destination -- Full navigation preserves transfer unload warnings and isolates library state. */
...
function RelayWorkspace() {
  ...
  const pageTitle = !session && accountSpaceId !== undefined ? "Library" : filter === "trash" ? "Trash" : currentAlbum ? currentAlbum.name : libraryQuery.album === "unorganised" ? "Unorganised" : filter === "all" ? (session?.space.kind === "personal" ? "My space" : "Shared library") : filter === "original" ? "Originals" : "Final cuts";
  ...
  return <div className={`app-shell ${transfers.length || download ? "has-transfers" : ""}`} onDragEnter={event => { if (event.dataTransfer.types.includes("Files")) { event.preventDefault(); dragDepth.current++; setDragging(true); } }} onDragOver={event => event.preventDefault()} onDragLeave={event => { event.preventDefault(); dragDepth.current--; if (dragDepth.current <= 0) setDragging(false); }} onDrop={event => { event.preventDefault(); dragDepth.current = 0; setDragging(false); if (session) void selectOriginals(event.dataTransfer.files); }}>
    ...
    <aside ref={sidebarRef} className={`sidebar${navigationOpen ? " is-open" : ""}`} role={navigationOpen ? "dialog" : undefined} aria-modal={navigationOpen || undefined} aria-label="Workspace navigation" onClick={event => { if ((event.target as HTMLElement).closest("button") && !(event.target as HTMLElement).closest(".library-switcher")) setNavigationOpen(false); }}>
      ...
      <div className="space-label"><span className="space-avatar">{(session?.space.name || "Your space").slice(0, 1).toUpperCase()}</span><div><strong>{session?.space.name || (accountSpaceId !== undefined ? "Your library" : "Your shared space")}</strong><span>{session ? session.space.kind === "personal" ? "Personal / Only you" : accountSpaceId !== undefined ? `${session.role === "owner" ? "Owner" : "Member"} / Account access` : "Connected workspace" : "A little less back and forth"}</span></div></div>
      <p className="nav-label">WORKSPACE</p>
      {accountSpaceId !== undefined && accountLibraries.length > 0 && <div className="library-switcher"><WorkspaceSelect label="Switch library" value={accountSpaceId} options={accountLibraries.map(space => ({ value: space.id, label: space.name, group: space.kind === "personal" ? "Personal" : "Shared" }))} onChange={spaceId => { if (spaceId !== accountSpaceId) window.location.assign(`/?space=${encodeURIComponent(spaceId)}`); }} /><p>Uploads keep their destination. Resume after switching.</p></div>}
    ...
    <div className="workspace" inert={navigationOpen}>
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        {loading ? <div className="loading-panel"><LoaderCircle className="spin" size={25} /><p>Opening your space…</p></div> : sessionFailure && !session ? <section className="loading-panel"><p>Couldn&apos;t open your space.</p>{accountSpaceId !== undefined && <a className="button secondary" href="/account">Sign in or choose a library</a>}<button className="button secondary" onClick={() => void loadSession()}>Try again</button></section> : invitationRequired ?
          ...
          {filter === "all" && !search && !libraryQuery.album && !libraryQuery.from && !libraryQuery.to && !libraryQuery.batch && !libraryQuery.type && !libraryQuery.uploader && <LibraryOverview albums={albums} interrupted={transfers.filter(transfer => transfer.deviceId === session.deviceId && transfer.accountSpaceId === accountSpaceId && ["paused", "error", "needs-file"].includes(transfer.state)).length} openAlbum={album => changeLibraryQuery({ ...emptyLibraryQuery, album })} reviewTransfers={() => { const tray = document.getElementById("file-transfers"); tray?.focus(); tray?.scrollIntoView({ block: "nearest" }); }} />}
          <LibraryTools feedback={{ message: feedbackMessage, setMessage: setFeedbackMessage, undo: undoLibraryAction, setUndo: setUndoLibraryAction }} albums={albums} sections={sections} query={libraryQuery} onQuery={changeLibraryQuery} isOwner={isOwner} selected={visibleItems.filter(item => selectedIds.has(item.id))} loadedCount={visibleItems.length} onSelectLoaded={() => setSelectedIds(new Set(visibleItems.slice(0, 100).map(item => item.id)))} onClearSelection={() => setSelectedIds(new Set())} refresh={refreshLibrary} renameItems={renameItems} onRename={setRenameItems} dateItem={dateItem} onDate={setDateItem} viewControls={<><SavedLibraryViews key={session.deviceId} storageKey={savedLibraryViewsKey(session.space.id, session.deviceId, session.authentication || "legacy")} current={{ category: filter, search, query: libraryQuery }} apply={view => { setFilter(view.category); setSearch(view.search); changeLibraryQuery(view.query); }} /><div className="view-toggle" role="group" aria-label="Library view"><button type="button" aria-label="Grid view" aria-pressed={libraryView === "grid"} onClick={() => chooseLibraryView("grid")}><Grid2X2 size={17} aria-hidden="true" /><span>Grid</span></button><button type="button" aria-label="List view" aria-pressed={libraryView === "list"} onClick={() => chooseLibraryView("list")}><List size={18} aria-hidden="true" /><span>List</span></button></div></>}>
          ...
          {!feedReady ? <section className="loading-panel" role="status">{currentFeedFailure ? <p>Your files will appear when the connection recovers.</p> : <><LoaderCircle className="spin" size={25} /><p>Loading files...</p></>}</section> : visibleItems.length ? <><div className="feed-label"><span>{isOwner && <SelectionControl label={`Select loaded files (${Math.min(visibleItems.length, 100)})`} checked={visibleItems.slice(0, 100).every(item => selectedIds.has(item.id))} mixed={visibleItems.some(item => selectedIds.has(item.id)) && !visibleItems.slice(0, 100).every(item => selectedIds.has(item.id))} disabled={mediaActionBusy} onToggle={() => setSelectedIds(visibleItems.slice(0, 100).every(item => selectedIds.has(item.id)) ? new Set() : new Set(visibleItems.slice(0, 100).map(item => item.id)))} />}{filter === "trash" ? "Removed files" : search ? "Search results" : "Recent additions"}<span className="inline-drop-hint">{filter !== "trash" && <><Upload size={13} />Drop files anywhere to add</>}</span></span><span aria-live="polite">{visibleItems.length} of {total} files{feedBusy ? " · Updating…" : ""}</span></div><div className={`media-grid${libraryView === "list" ? " media-list" : ""}${selectedIds.size ? " has-selection" : ""}`}>{visibleItems.map(item => <article className={`media-card${selectedIds.has(item.id) ? " is-selected" : ""}`} key={item.id}><button className="media-cover" onClick={() => setModal(item)} aria-label={`Preview ${item.name}`}><MediaPreview item={item} /><span className={`category-badge ${item.category}`}>{item.category === "final" ? <CheckCheck size={12} /> : <ShieldCheck size={12} />}{item.category === "final" ? "Final cut" : "Original"}</span>{item.mime.startsWith("video/") && <span className="play-badge"><Play size={14} fill="currentColor" /></span>}</button><div className="media-card-body"><div className="card-name-row">{isOwner && <SelectionControl className="file-selection" label={`Select ${item.name}`} checked={selectedIds.has(item.id)} disabled={mediaActionBusy} onToggle={shift => toggleSelection(item.id, shift)} />}<h3 title={item.name}>{item.name}</h3></div><p>{formatBytes(item.size)}<span>·</span>{item.deviceName}<span className="list-category">{item.category === "final" ? "Final cut" : "Original"}</span></p><div className="card-bottom"><span className="file-date">{libraryQuery.dateMode === "captured" && item.capturedAt ? `Taken ${item.capturedAt.slice(0, 10)}` : `Uploaded ${new Date(item.createdAt).toISOString().slice(0, 10)}${libraryQuery.dateMode === "captured" ? " - date taken unknown" : ""}`}</span></div><div className="item-actions" aria-label={`Actions for ${item.name}`}>
                ...
                {isOwner && filter !== "trash" && <button className="icon-button" aria-label="Rename" title="Rename" disabled={mediaActionBusy} onClick={() => setRenameItems([item])}><Pencil size={16} /></button>}
                {isOwner && (filter === "trash" ? <><button className="icon-button" aria-label="Restore" title="Restore" disabled={mediaActionBusy} onClick={() => void changeMedia(item, "restore")}><Undo2 size={17} /></button><button className="icon-button item-trash" aria-label="Delete permanently" title="Delete permanently" disabled={mediaActionBusy} onClick={event => { event.currentTarget.focus(); void changeMedia(item, "delete"); }}><Trash2 size={17} /></button></> : <button className="icon-button item-trash" aria-label="Move to Trash" title="Move to Trash" disabled={mediaActionBusy} onClick={() => void changeMedia(item, "archive")}><Trash2 size={17} /></button>)}
              </div></div></article>)}</div>{nextCursor && <div className="load-more"><button className="button secondary" disabled={feedBusy} onClick={() => void loadMore()}>{feedBusy ? "Loading…" : "Load more files"}</button></div>}</> : <section className="empty-feed"><div className="empty-art" aria-hidden="true"><span className="art-card back"><Clapperboard size={31} strokeWidth={1.2} /></span><span className="art-card front"><FileImage size={35} strokeWidth={1.2} /><span /><span /></span><span className="art-arrow"><ArrowDown size={18} /></span></div><h2>{search ? "No matching files." : filter === "trash" ? "Nothing in Trash." : libraryQuery.album || libraryQuery.from || libraryQuery.to || libraryQuery.batch || libraryQuery.type || libraryQuery.uploader ? "No files in this view." : filter === "final" ? "Ready for the final touch." : "A fresh space for your next idea."}</h2><p>{search ? "Try a different filename or clear your search." : filter === "trash" ? "Removed files appear here until you restore or permanently delete them." : libraryQuery.from || libraryQuery.to || libraryQuery.batch || libraryQuery.type || libraryQuery.uploader ? "Try a different file type, uploader or date range, or clear the filters above." : libraryQuery.album ? "Add files here, or select existing files in All files and add them to an album." : filter === "final" ? "Drop your finished edits here. Everyone can save the original export." : (session.space.kind === "personal" ? "Your originals, in your own space. Shared-library members cannot see files here." : "Add your first photos or videos. They'll be right here for every connected device.")}</p>{search || libraryQuery.from || libraryQuery.to || libraryQuery.batch || libraryQuery.type || libraryQuery.uploader ? <button className="button secondary empty-primary" onClick={() => { setSearch(""); changeLibraryQuery({ ...libraryQuery, from: "", to: "", batch: "", type: "", uploader: "" }); }}>Clear search and filters</button> : filter !== "trash" && <button className="button primary empty-primary" disabled={session.transport === "unconfigured" || Boolean(currentAlbum?.archivedAt)} onClick={() => fileInput.current?.click()}><Plus size={18} />{currentAlbum ? `Add files to ${currentAlbum.name}` : "Add your first files"}</button>}<span className="empty-footnote"><ShieldCheck size={14} />No compression. No extra steps.</span></section>}

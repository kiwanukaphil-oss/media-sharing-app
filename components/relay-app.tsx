...
function RelayWorkspace() {
  ...
  const pageTitle = filter === "trash" ? "Trash" : currentAlbum ? currentAlbum.name : libraryQuery.album === "unorganised" ? "Unorganised" : filter === "all" ? (session?.space.kind === "personal" ? "My space" : "Shared library") : filter === "original" ? "Originals" : "Final cuts";
  ...
  return <div className={`app-shell ${transfers.length || download ? "has-transfers" : ""}`} onDragEnter={event => { if (event.dataTransfer.types.includes("Files")) { event.preventDefault(); dragDepth.current++; setDragging(true); } }} onDragOver={event => event.preventDefault()} onDragLeave={event => { event.preventDefault(); dragDepth.current--; if (dragDepth.current <= 0) setDragging(false); }} onDrop={event => { event.preventDefault(); dragDepth.current = 0; setDragging(false); if (session) void selectOriginals(event.dataTransfer.files); }}>
    ...
    <aside ref={sidebarRef} className={`sidebar${navigationOpen ? " is-open" : ""}`} role={navigationOpen ? "dialog" : undefined} aria-modal={navigationOpen || undefined} aria-label="Workspace navigation" onClick={event => { if ((event.target as HTMLElement).closest("button") && !(event.target as HTMLElement).closest(".library-switcher")) setNavigationOpen(false); }}>
      ...
      <nav aria-label={session?.space.kind === "personal" ? "Personal media" : "Shared media"}>

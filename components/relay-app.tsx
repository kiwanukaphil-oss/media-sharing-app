...
// LibraryOverview is a retirement candidate: arrival now shows closed albums only.
import { AlbumLibrary, AlbumCover, useAlbumAppearance } from "./album-library";
...
function RelayWorkspace() {
  ...
  const [sectionMoveItems, setSectionMoveItems] = useState<MediaItem[]>([]);
  const [sections, setSections] = useState<AlbumSection[]>([]);
  const [librarySurface, setLibrarySurface] = useState<"albums" | "files">("albums");
  useEffect(() => { document.getElementById("main-content")?.focus({ preventScroll: true }); }, [librarySurface]);
  const [albumFailure, setAlbumFailure] = useState("");
  const { appearance, updateAppearance } = useAlbumAppearance(`relay-album-appearance:${session?.space.id ?? "none"}:${session?.authentication ?? "legacy"}:${session?.deviceId ?? "none"}`);
  ...
  // Preserve an explicit retry state rather than mistaking a failed album read for an empty collection.
  const refreshAlbums = useCallback(async (signal?: AbortSignal) => {
    try {
      const result = await requestJson<{ albums: Album[]; sections?: AlbumSection[] }>("albums", { signal });
      if (signal?.aborted || libraryAccessLost.current) return;
      setAlbums(result.albums); setSections(result.sections || []); setAlbumFailure("");
    } catch (failure) {
      if (!signal?.aborted) setAlbumFailure(failure instanceof Error ? failure.message : "Please try again.");
      throw failure;
    }
  }, [requestJson]);
  ...
  useEffect(() => {
    ...
    const restoreLibraryLocation = () => {
      ...
      const explicitFileRoute = params.get("view") === "files" || Boolean(restored.album || restored.from || restored.to || restored.batch || restored.type || restored.uploader || params.get("q") || category !== "all");
      setLibrarySurface(params.get("view") === "albums" ? "albums" : explicitFileRoute ? "files" : "albums");
      setSelectedIds(new Set()); setModal(null);
  ...
  }, [accountSpaceId, filter, search, libraryQuery, librarySurface, session, refreshFeed, memory]);
  useEffect(() => {
  ...
  const fileViewControls = <div className="view-toggle" role="group" aria-label="Library view"><button type="button" aria-label="Grid view" title="Grid view" aria-pressed={libraryView === "grid"} onClick={() => chooseLibraryView("grid")}><Grid2X2 size={17} aria-hidden="true" /><span>Grid</span></button><button type="button" aria-label="List view" title="List view" aria-pressed={libraryView === "list"} onClick={() => chooseLibraryView("list")}><List size={18} aria-hidden="true" /><span>List</span></button></div>;
  ...
  async function selectOriginals(selected: FileList | File[]) {
    ...
    if (librarySurface === "albums") { setError("Open or create an album before adding files."); return; }
  ...
  function changeLibraryQuery(query: LibraryQuery) {
    ...
    setSectionMoveItems([]);
    history.pushState(null, "", `${location.pathname}?${new URLSearchParams({ category: filter, q: search, ...query, view: "files", ...(accountSpaceId === undefined ? {} : { space: accountSpaceId }) })}`);
    setLibrarySurface("files"); setLibraryQuery(query); setSelectedIds(new Set());
  }
  function openAlbumLibrary() {
    rememberAlbumPosition();
    setSectionMoveItems([]);
    history.pushState(null, "", `${location.pathname}?${new URLSearchParams({ view: "albums", ...(accountSpaceId === undefined ? {} : { space: accountSpaceId }) })}`);
    setLibrarySurface("albums"); setLibraryQuery(emptyLibraryQuery); setFilter("all"); setSearch("");
    setSelectedIds(new Set()); setModal(null); setRenameItems([]); setDateItem(null);
    document.getElementById("main-content")?.focus();
  }
  async function openCreatedAlbum(id: string, message: string) {
    setFeedbackMessage(message); setUndoLibraryAction(null); setFilter("all"); setSearch("");
    changeLibraryQuery({ ...emptyLibraryQuery, album: id });
    await refreshAlbums().catch(() => {});
  }
  ...
  const canUpload = Boolean(session && session.role !== "viewer");
  ...
  return <div className={`app-shell album-first ${librarySurface === "albums" ? "albums-arrival" : "album-files"}${currentAlbum ? " in-album" : ""} ${transfers.length || download ? "has-transfers" : ""}`} onDragEnter={event => { if (canUpload && librarySurface === "files" && event.dataTransfer.types.includes("Files")) { event.preventDefault(); dragDepth.current++; setDragging(true); } }} onDragOver={event => event.preventDefault()} onDragLeave={event => { event.preventDefault(); dragDepth.current--; if (dragDepth.current <= 0) setDragging(false); }} onDrop={event => { event.preventDefault(); dragDepth.current = 0; setDragging(false); if (canUpload) void selectOriginals(event.dataTransfer.files); }}>
    ...
    <aside ref={sidebarRef} className={`sidebar${navigationOpen ? " is-open" : ""}`} role={navigationOpen ? "dialog" : undefined} aria-modal={navigationOpen || undefined} aria-label="Workspace navigation" onClick={event => { if ((event.target as HTMLElement).closest("button") && !(event.target as HTMLElement).closest(".library-switcher")) setNavigationOpen(false); }}>
      ...
      <nav aria-label={!session ? "Media" : session.space.kind === "personal" ? "Personal media" : "Shared media"}>
        <button className={`nav-item ${librarySurface === "albums" ? "active" : ""}`} aria-current={librarySurface === "albums" ? "page" : undefined} onClick={openAlbumLibrary}><Folder size={18} />Albums<span>{albums.filter(album => !album.archivedAt).length}</span></button>
        {/* The broad file feed is retained for compatibility behind a deliberate secondary route. */}
        <details className="library-utilities"><summary>Browse files</summary>
        <button aria-pressed={librarySurface === "files" && filter === "all" && !libraryQuery.album} className={`nav-item ${librarySurface === "files" && filter === "all" && !libraryQuery.album ? "active" : ""}`} onClick={() => { setFilter("all"); setSearch(""); changeLibraryQuery(emptyLibraryQuery); }}><Grid2X2 size={18} />All files<span>{counts.all}</span></button>
        {/* Retirement candidate: global categories are shown only for explicit legacy deep links; sections own the current album workflow. */}
        {!currentAlbum && (filter === "original" || filter === "final") && <details className="category-navigation"><summary>File labels</summary>
        <button aria-pressed={filter === "original" && !libraryQuery.album} className={`nav-item ${filter === "original" && !libraryQuery.album ? "active" : ""}`} onClick={() => { setFilter("original"); setSearch(""); changeLibraryQuery(emptyLibraryQuery); }}><FileImage size={18} />Originals<span>{originalCount}</span></button>
        <button aria-pressed={filter === "final" && !libraryQuery.album} className={`nav-item ${filter === "final" && !libraryQuery.album ? "active" : ""}`} onClick={() => { setFilter("final"); setSearch(""); changeLibraryQuery(emptyLibraryQuery); }}><Clapperboard size={18} />Final cuts<span>{finalCount}</span></button>
        </details>}
        </details>
      </nav>
      {session && <><p className="nav-label albums-label">ALBUMS</p><nav className="album-navigation" aria-label="Albums">
        {albums.filter(album => !album.archivedAt).map(album => <button className={`nav-item${libraryQuery.album === album.id ? " active" : ""}`} aria-current={libraryQuery.album === album.id ? "page" : undefined} key={album.id} onPointerMove={() => memory.warmView?.(album.id)} onPointerLeave={() => memory.cancelWarm()} onFocus={() => memory.warmView?.(album.id)} onBlur={() => memory.cancelWarm()} onClick={() => { setFilter("all"); setSearch(""); changeLibraryQuery({ ...emptyLibraryQuery, album: album.id }); }}><Folder size={18} /><span className="album-name">{album.name}</span><span>{album.count}</span></button>)}
    ...
    <div className="workspace" inert={navigationOpen}>
      <header className="topbar"><div className="breadcrumb"><button ref={navigationButton} className="icon-button navigation-open" aria-label="Open navigation" aria-expanded={navigationOpen} onClick={() => setNavigationOpen(true)}><Menu size={20} /></button><span title={session?.space.name}>{session?.space.name || "Workspace"}</span><ChevronRight size={14} /><strong>{pageTitle}</strong></div><div className="topbar-right"><HideLibraryButton /><button className="icon-button mobile-help" aria-label="Help" onClick={() => setModal("help")}><CircleHelp size={20} /></button><span className="connection"><span className={`status-dot ${session && online ? "online" : ""}`} />{loading ? "Opening…" : session ? online ? "Connected" : "Reconnecting" : accountSpaceId !== undefined ? "Not connected" : "Not paired"}</span><a className="button secondary compact" href="/account">Account</a>{accountSpaceId === undefined && <button className="button secondary compact" aria-label={isOwner ? "Pair device" : "Devices"} disabled={!session} onClick={() => setModal("devices")}><MonitorSmartphone size={16} /><span>{isOwner ? "Pair device" : "Devices"}</span></button>}</div></header>
      <main id="main-content" tabIndex={-1}>
        ...
        {librarySurface === "files" || !session ? <div className="page-heading"><div className="album-title-group">{currentAlbum && <AlbumCover album={currentAlbum} colour={appearance[currentAlbum.id]?.colour} compact />}<h1>{pageTitle}<span className="title-dot">.</span></h1></div>{filter !== "trash" && canUpload && <div className="page-upload-actions"><button className="button primary" disabled={!session || session.transport === "unconfigured" || Boolean(currentAlbum?.archivedAt)} onClick={() => fileInput.current?.click()}><Plus size={18} />Add files</button></div>}{filter === "trash" && <button className="button secondary" onClick={() => setFilter("all")}>Back to files</button>}</div> : null}
        ...
        {loading ? <div className="loading-panel"><LoaderCircle className="spin" size={25} /><p>Opening your space…</p></div> : sessionFailure && !session ? <section className="loading-panel"><p>Couldn&apos;t open your space.</p>{accountSpaceId !== undefined && <a className="button secondary" href="/account">Sign in or choose a library</a>}<button className="button secondary" onClick={() => void loadSession()}>Try again</button></section> : invitationRequired ?
          <section className="welcome-panel"><div className="welcome-symbol"><Link2 size={32} /></div><h2>Open a library</h2><p>Sign in or use an invitation link.</p><div className="welcome-actions"><a className="button primary" href="/account">Account &amp; libraries</a><button className="button secondary" onClick={() => setModal("help")}><CircleHelp size={17} />Help</button></div></section> : !session ?
          <section className="welcome-panel"><div className="welcome-symbol"><ArrowLeftRight size={32} /></div><h2>{invitation ? "Join shared space" : "Create shared space"}</h2><form onSubmit={connectSpace}>{!invitation && <label>Space name<input required maxLength={60} value={spaceName} onChange={event => setSpaceName(event.target.value)} /></label>}<label>Device name<input required maxLength={60} value={deviceName} onChange={event => setDeviceName(event.target.value)} /></label><button className="button primary" disabled={connecting}>{connecting ? <LoaderCircle size={18} className="spin" /> : <ArrowUpRight size={18} />}{invitation ? "Join shared space" : "Create shared space"}</button></form><small>Access is managed by the library owner.</small></section> : librarySurface === "albums" ? <AlbumLibrary albums={albums} sections={sections} spaceName={session.space.name} canOrganise={isOrganiser} appearance={appearance} onAppearance={updateAppearance} onOpen={album => { setFilter("all"); setSearch(""); changeLibraryQuery({ ...emptyLibraryQuery, album }); }} onCreated={openCreatedAlbum} onUnorganised={() => { setFilter("all"); setSearch(""); changeLibraryQuery({ ...emptyLibraryQuery, album: "unorganised" }); }} failure={albumFailure} retry={() => void refreshAlbums().catch(() => {})} /> : <>
          {currentAlbum && <div className="album-destination" role="status"><Folder size={16} /><span>{currentAlbum.archivedAt ? "Archived album" : `Adding to ${sections.find(section => section.id === libraryQuery.section)?.name || "Unsectioned"}`}</span></div>}
          {currentAlbum && (filter === "original" || filter === "final") && <button className="filter-chip" onClick={() => setFilter("all")}>Clear legacy file filter<X size={13} /></button>}
          {albumFailure && <div className="error-banner" role="alert">{albumFailure}<button className="button secondary" onClick={() => void refreshAlbums().catch(() => {})}>Retry albums</button></div>}
          <LibraryTools onMoveSection={setSectionMoveItems} feedback={{ message: feedbackMessage, setMessage: setFeedbackMessage, undo: undoLibraryAction, setUndo: setUndoLibraryAction }} albums={albums} sections={sections} query={libraryQuery} onQuery={query => { if (query.album && query.section !== libraryQuery.section) setFilter("all"); setSectionMoveItems([]); changeLibraryQuery(query); }} canOrganise={isOrganiser} canSelectFiles={canSelectFiles} selected={visibleItems.filter(item => selectedIds.has(item.id))} loadedCount={visibleItems.length} onSelectLoaded={() => setSelectedIds(new Set(visibleItems.slice(0, 100).map(item => item.id)))} onClearSelection={() => setSelectedIds(new Set())} refresh={refreshLibrary} renameItems={renameItems} onRename={setRenameItems} dateItem={dateItem} onDate={setDateItem} viewControls={<><SavedLibraryViews key={session.deviceId} storageKey={savedLibraryViewsKey(session.space.id, session.deviceId, session.authentication || "legacy")} current={{ category: filter, search, query: libraryQuery }} apply={view => { setFilter(view.category); setSearch(view.search); changeLibraryQuery(view.query); }} />{!currentAlbum && fileViewControls}</>}>
            <label className="search-field"><span className="visually-hidden">Search filenames</span><Search className="search-symbol" size={18} aria-hidden="true" /><input type="search" placeholder={currentAlbum ? "Search this album…" : "Find a file…"} value={search} onChange={event => setSearch(event.target.value)} />{search && <button className="icon-button clear-search" aria-label="Clear search" onClick={() => setSearch("")}><X size={17} /></button>}</label>
          </LibraryTools>
    ...
    {modal === "devices" && <ModalFrame title="Your connected devices" onClose={() => setModal(null)}>
      ...
      {isOwner && <div className="pair-panel"><Link2 size={22} /><h3>Pair device</h3>

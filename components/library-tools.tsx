...
export function LibraryTools(props: Props) {
  ...
  const editableSelection = props.canOrganise || (props.selected.length > 0 && props.selected.every(file => Boolean(file.canEdit)));
  ...
  const activeFilterCount = Number(Boolean(query.from || query.to)) + Number(Boolean(query.batch)) + Number(query.dateMode === "captured") + Number(Boolean(query.type)) + Number(Boolean(query.uploader)) + Number(Boolean(query.favorites));
  ...
  return <section className="library-tools" aria-label="Library organisation">
    ...
    <div className="library-filters">
      {props.canFavorite && <div className="filter-field"><span>Personal view</span><WorkspaceSelect label="Personal view" value={query.favorites} onChange={favorites => onQuery({ ...query, favorites })} options={[{ value: "", label: "All accessible files" }, { value: "1", label: "My favourites" }]} /></div>}
    ...
    {(canOrganise || props.canSelectFiles) && <div className={`selection-toolbar${selected.length ? " has-selection" : ""}`} aria-label="Bulk file actions">
      {!!selected.length && <><strong aria-live="polite">{selected.length} selected</strong>{props.canEditFiles && !editableSelection && <span role="status">Contributors can edit only their own files. Adjust your selection.</span>}
        ...
        </>}{selected.length===1&&!selected[0].archivedAt&&<DuplicateReview key={selected[0].id} source={selected[0]} albums={albums} canOrganise={canOrganise} refresh={refresh}/>}<MetadataExport files={selected} /><OriginalPackage files={selected} />{props.onCreateDelivery && <button className="button secondary compact" disabled={busy} onClick={() => props.onCreateDelivery?.(selected)}>Create delivery</button>}

...
import { ArrowUpRight, Folder, Grid2X2, Inbox, List, Pin, Plus, Search, X } from "lucide-react";
...
export function AlbumCover({ album, colour, compact = false }: { album: Pick<Album, "id" | "createdAt">; colour?: number; compact?: boolean }) {
  ...
  return <div className={`collection-cover cover-art-${index}${compact ? " compact-cover" : ""}`} style={{ "--cover": palette.background, "--cover-ink": palette.ink, "--cover-shape": palette.shape } as CSSProperties} aria-hidden="true">
    ...
    <span className="cover-year">{new Date(album.createdAt).getFullYear().toString().slice(-2)}</span><span className="cover-caption">{palette.name} / ALBUM</span>
  </div>;
}
...
export function AlbumLibrary(props: Props) {
  ...
  return <section className="album-library" aria-label="Album library">
    ...
    {creating && <CreateAlbumDialog accessScopeId={props.accessScopeId} spaceName={props.spaceName} onClose={() => setCreating(false)} onCreated={props.onCreated} onAppearance={props.onAppearance} />}
  </section>;
}

// Album creation is durable before optional sections; partial template failures never duplicate an album.
function CreateAlbumDialog({ accessScopeId, spaceName, onClose, onCreated, onAppearance }: Pick<Props, "spaceName" | "onCreated" | "onAppearance" | "accessScopeId"> & { onClose: () => void }) {
  ...
  async function createCollection(event: React.FormEvent) {
    ...
    try {
      const result = await requestJson<{ id: string }>("albums", { method: "POST", body: JSON.stringify({ name: name.trim(), description: description.trim(), accessScopeId: accessScopeId ?? null }) });

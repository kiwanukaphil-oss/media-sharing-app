...
export default function SpacePeople({ spaceId }: { spaceId: string }) {
  ...
  async function changeMember(member: Member, action: "owner" | "member" | "editor" | "remove" | "leave") {
    const removing = action === "remove" || action === "leave";
    const title = action === "leave" ? `Leave ${people?.space.name}?` : action === "remove" ? `Remove ${member.name}?` : `Make ${member.name} ${action === "owner" ? "an owner" : action === "editor" ? "an editor" : "a member"}?`;

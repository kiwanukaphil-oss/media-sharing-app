...
import { ArrowDown, ArrowUp, FolderInput, Image as ImageIcon, LayoutTemplate, MoreHorizontal, Pencil, Plus, Trash2, X } from "lucide-react";
...
export function AlbumSections({ album, sections, query, onQuery, selected, canOrganise, canMoveSelection, refresh, clearSelection, feedback }: Props) {
  ...
  // The original inline move dialog is a retirement candidate; file and selection actions now use SectionPlacement.
  const legacyInlineMove = false;
  ...
  return <section className="album-sections" aria-label="Album sections">
    <div className="section-navigation">
      ...
      {legacyInlineMove && (canEdit || (canMoveSelection && !album.archivedAt)) && selected.length > 0 && !selected.some(file => file.archivedAt) && <button className="button compact" onClick={() => { setFailure(""); setDestination(""); setEditor("move"); }}><FolderInput size={16} />Move to section</button>}
    </div>
    {/* The former explanatory paragraph is a retirement candidate; placement and access are shown in their relevant dialogs. */}

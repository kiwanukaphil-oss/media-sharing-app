...
export function fileEditAuthority(device: ActiveDevice, alias: "media" | "candidate" | "m" = "media") {
  ...
  if (device.authentication !== "account") return { sql: `(${organiser.sql}) AND ${audience.sql}`, bindings: [...organiser.bindings, ...audience.bindings] };
  ...
  return { sql: `((${organiser.sql}) OR ((${upload.sql}) AND EXISTS (
    ...
    AND (${alias}.device_id=editor_actor.device_id OR EXISTS (SELECT 1 FROM legacy_owner_claims claim
      WHERE claim.membership_id=editor_member.id AND claim.device_id=${alias}.device_id))))) AND ${audience.sql}`,
    bindings: [...organiser.bindings, ...upload.bindings, device.id, ...audience.bindings] };
}

...
export function intakeIssuerAuthority(now=Date.now(),state:"draft"|"open"="open") {
  return {sql:`upload_requests.state=? AND upload_requests.revoked_at IS NULL AND upload_requests.expires_at>?
    AND EXISTS(SELECT 1 FROM space_memberships issuer JOIN people owner ON owner.id=issuer.person_id
      ...
      AND (upload_requests.access_scope_id IS NULL OR EXISTS(SELECT 1 FROM scope_grants grant WHERE grant.scope_id=upload_requests.access_scope_id AND grant.membership_id=issuer.id AND grant.revoked_at IS NULL)))`,bindings:[state,now]};
}
...
export function intakeRecipientAuthority(session:AccountSession,now=Date.now()) {
  const issuer=intakeIssuerAuthority(now),closure=accountClosureCommitAuthority(session);
  return {sql:`(${issuer.sql}) AND upload_requests.accepted_by=? AND EXISTS(SELECT 1 FROM account_sessions current
    ...
    AND current.authenticated_at>=recipient.credentials_changed_at) AND ${closure.sql}`,bindings:[...issuer.bindings,session.personId,session.sessionId,session.personId,now,...closure.bindings]};
}
...
export async function acceptUploadRequest(database:D1Database,session:AccountSession,tokenHash:string,now=Date.now()) {
  ...
  await database.prepare(`UPDATE upload_requests SET accepted_by=?,accepted_at=?,revision=revision+1
    ...
    AND EXISTS(SELECT 1 FROM account_sessions current JOIN people recipient ON recipient.id=current.person_id
      ...
      AND lower(recipient.verified_email)=lower(upload_requests.recipient_email)) AND ${closure.sql}`)
    .bind(session.personId,now,tokenHash,...issuer.bindings,session.sessionId,session.personId,now,...closure.bindings).run();

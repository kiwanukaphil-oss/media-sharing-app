...
type Session = Pick<AccountSession, "personId" | "sessionId"> & { closureAdmissionId?: string };
const ownerAuthority = `EXISTS (SELECT 1 FROM space_memberships m JOIN people p ON p.id = m.person_id
  ...
  AND a.revoked_at IS NULL AND a.expires_at > ? AND a.authenticated_at >= p.credentials_changed_at
...
export async function revokeLegacyAccess(database: D1Database, session: Session, spaceId: string, targetId: string | null, now = Date.now()) {
  ...
  const results = await database.batch([
    database.prepare(`UPDATE devices SET revoked_at = ? WHERE space_id = ? AND (? IS NULL OR id = ?)
      AND revoked_at IS NULL AND expires_at > ? AND ${ownerAuthority} AND ${admission.sql}`)
      .bind(now, spaceId, targetId, targetId, now, session.personId, session.sessionId, now, ...admission.bindings),
    database.prepare(`UPDATE invitations SET expires_at = 0 WHERE space_id = ? AND redeemed_at IS NULL
      AND created_by IN (SELECT id FROM devices WHERE space_id = ? AND revoked_at IS NOT NULL AND ${ownerAuthority}) AND ${admission.sql}`)
      .bind(spaceId, spaceId, session.personId, session.sessionId, now, ...admission.bindings),
  ]);

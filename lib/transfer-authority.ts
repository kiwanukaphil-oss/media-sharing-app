...
function liveTransferAuthority(device: ActiveDevice, now = Date.now(), ownerOnly: boolean | "organiser" | "read" = false) {
  if (device.authentication === "account") {
    ...
    return { sql: `EXISTS (SELECT 1 FROM account_space_actors actor
      ...
      AND m.revoked_at IS NULL AND m.role IN ('owner','member','editor','contributor'${ownerOnly === "read" ? ",'viewer'" : ""}) AND p.disabled_at IS NULL AND a.revoked_at IS NULL AND a.expires_at>?
      ${ownerOnly === "organiser" ? "AND m.role IN ('owner','editor')" : ownerOnly === true ? "AND m.role='owner'" : ""}
  ...
  return { sql: `EXISTS (SELECT 1 FROM devices d WHERE d.id=? AND d.space_id=?
    ...
    ${ownerOnly === true || ownerOnly === "organiser" ? "AND d.role='owner'" : ""}
...
export function transferAuthority(device: ActiveDevice, now = Date.now(), ownerOnly: boolean | "organiser" | "read" = false) {

...
export async function createImportLayout(request: Request, device: ActiveDevice) {
  ...
  const input = await readJson(request, z.object({ id: z.string().uuid(), name, sections: z.array(z.object({ id: z.string().uuid(), name })).max(50) }));
  ...
  await db.batch([
    db.prepare(`INSERT OR IGNORE INTO albums(id,space_id,name,description,created_at) SELECT ?,?,?,'',? WHERE ${authority.sql}`)
      .bind(input.id, device.space_id, input.name, now, ...authority.bindings),
  ...
  const layout = await db.prepare(`SELECT id,name FROM albums WHERE id=? AND space_id=? AND name=? AND description='' AND revision=0
    AND archived_at IS NULL AND deleted_at IS NULL AND ${authority.sql}
    ...
    .bind(input.id, device.space_id, input.name, ...authority.bindings, device.id, input.sections.length, sections).first();

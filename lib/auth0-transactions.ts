...
export async function storeAuth0Transaction(database: D1Database, settings: Auth0Settings,
  ...
  const sessionMode = transaction.sessionMode ?? "temporary";
  accountSessionLifetime(sessionMode);
  ...
  await database.batch([
    ...
    database.prepare(`INSERT INTO auth_transactions (state_hash, browser_hash, configuration_hash, nonce, verifier, expires_at, session_mode)
      VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(stateHash, browserHash, configurationHash, transaction.nonce, transaction.verifier, transaction.expiresAt, sessionMode),
  ]);
}
...
export async function consumeAuth0Transaction(database: D1Database, settings: Auth0Settings,
  ...
  const stored = await database.prepare(`DELETE FROM auth_transactions
    ...
    RETURNING nonce, verifier, expires_at, session_mode`)
    .bind(stateHash, browserHash, configurationHash, now, now + AUTH0_TRANSACTION_LIFETIME_MS).first<StoredTransaction>();
  return stored ? { state, browserBinding, nonce: stored.nonce, verifier: stored.verifier, expiresAt: stored.expires_at, sessionMode: stored.session_mode } : null;
}

## 24 September 2026 - Cobalt UI live

Worker `27898165-96ee-4e28-a1ef-a428a9cd73f9` serves 100% from 03:48 UTC, source `37620f4` on `main`. The release applies the approved Cobalt navy, orange, blue and amber UI, preserves the app-first navigation/filter structure and replaces promotional copy with functional labels. The first hosted run caught a narrow-screen album-heading overflow; the next run exposed Chromium's repeated-folder-selection failure. Sources `9269935` and `37620f4` correct those issues, and hosted run `35952943796` passes both verification and the complete Chromium, Firefox and WebKit browser matrix. This was a code/assets-only deployment: schema remains 0020 and existing D1, R2, rate limits, identity pilot, 100 GiB combined storage, custom domain and backup coordination settings are unchanged. Both public origins pass health, dependency-readiness, security-header and anonymous-feed-denial probes. The live hashed stylesheet contains the Cobalt navy/orange tokens and mobile heading-wrap rule, with no Graphite primary token.

...
try {
  ...
  if(storagePoolChecks){const {verifyStoragePoolRoutes}=await import('../tests/storage-pool-routes.mjs');await verifyStoragePoolRoutes(database,(url,options)=>emulator.dispatchFetch(url,options));}
    ...
    for (const name of ['storage-summary-browser', 'album-library-browser', 'web-browser', 'device-access-browser', 'usability-browser', 'media-polish-browser', 'library-organisation-browser', 'album-sections-browser', 'account-browser', 'account-library-browser', 'workspace-navigation-browser', 'space-people-browser', 'publication-browser', 'retrieval-presentation-browser', 'editor-browser', 'favorites-browser', 'activity-browser', 'access-scopes-browser', 'intake-browser', 'delivery-browser', 'metadata-export-browser', 'original-package-browser', 'original-package-file-writer', 'duplicate-browser', 'folder-import-browser']) await runIntegrationTest(name);
  } else {

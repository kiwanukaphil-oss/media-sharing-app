...
// Rehearse only in memory, starting from a reviewed pre-delivery schema. Preserve every existing
// column/row, introduce no invitations, and verify schema triggers survive a quarantined restore.
export async function rehearseDeliveryUpgrade(sql){
  ...
  try{
    ...
    if(!reviewMinimisationSchema(db,shape()).accepted||names().includes('delivery_snapshots'))throw new Error('Expected a reviewed pre-delivery snapshot.');
    ...
    checkDatabase(db);
    for(const table of before)if(rows(table)!==table.digest)throw new Error('Existing rows changed during delivery migration.');
    for(const name of ['delivery_snapshots','delivery_items','delivery_recipients'])if(db.prepare(`SELECT COUNT(*) n FROM ${name}`).get().n)throw new Error('Migration invented delivery data.');
    if(!reviewMinimisationSchema(db,shape()).accepted)throw new Error('Delivery minimisation schema is unreviewed.');
    ...
    return {status:'verified-local-delivery-upgrade',existingTablesPreserved:before.length,noInvitationsCreated:true,restoredTriggersVerified:true,restoreQuarantineVerified:true,
      schemaDigest:digest(JSON.stringify(shape())),sourceDigest:digest(sql),remoteApplied:false};
  }finally{db.close();}
}
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href){
  try{console.log(JSON.stringify(await rehearseDeliveryUpgrade(await readFile(process.argv[2],'utf8'))));}
  catch(error){console.error('Delivery upgrade rehearsal failed: '+error.message);process.exitCode=1;}
}

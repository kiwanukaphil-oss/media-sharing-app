...
export function identityRuntimeVariables(configuration) {
  const fields = ['enabled', 'rollout', 'domain', 'clientId', 'appOrigin', 'personalStorageBudgetBytes'];
  if (!configuration || Object.keys(configuration).some(key => !fields.includes(key)) ||
      typeof configuration.enabled !== 'boolean' || !['pilot', 'open'].includes(configuration.rollout) || typeof configuration.domain !== 'string' ||
  ...
  return { AUTH0_ENABLED: String(configuration.enabled), AUTH0_ROLLOUT: configuration.rollout, AUTH0_DOMAIN: configuration.domain.toLowerCase(),

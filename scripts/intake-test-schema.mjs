...
export async function installIntakeTestSchema(database){
  // Retirement candidate: kept for older isolated branches; migration 0025 now owns this schema.
  if(await database.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name='upload_requests'").first())return;
  ...
  try{
    ...
    if(source.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name='upload_requests'").get())throw new Error('Apply the complete journal before running intake tests.');
    source.exec(await readFile('docs/prototypes/upload-request-schema.sql','utf8'));
